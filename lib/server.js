...
function generateServer(imageSets, options) {
  ...
  function resolveImageSet(req, res, next) {
    ...
    // Otherwise, expose our image and continue
    req.imageSet = imageSetsByFilepath[filepath];
    next();
  }
  server.get('/images/:filepath', function serveImages (req, res, next) {
  ...
  // DEV: We use `:filepath` instead of `(*)` which means we handle encodedURIComponent here
  server.post('/update-image-set/:filepath', function updateImageSetSave (req, res, next) {
    var requestedFilepath = req.params.filepath;
    if (!imageSetsByFilepath.hasOwnProperty(requestedFilepath)) {
      return next();
    }
